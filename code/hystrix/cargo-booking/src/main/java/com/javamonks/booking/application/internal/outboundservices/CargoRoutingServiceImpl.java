...
public class CargoRoutingServiceImpl implements CargoRoutingService {
	...
	/*
	 * @HystrixCommand(fallbackMethod = "findOptimalRouteFallback",commandKey =
	 * "rskey", threadPoolKey = "rskey", commandProperties = {
	 * 
	 * @HystrixProperty(name = "execution.timeout.enabled", value = "false") })
	 */

	@HystrixCommand(fallbackMethod = "findOptimalRouteFallback", commandProperties = {
			@HystrixProperty(name = "execution.timeout.enabled", value = "false") })
	public TransitPath findOptimalRoute(String originUnLocode, String destinationUnLocode, String deadline) {
		final String REST_URI = "http://routing-service/cargorouting/optimalRoute?origin={origin}&destination={destination}&deadline={deadline}";

		System.err.println("CargoRoutingServiceImpl.findOptimalRoute()======hitting cargorouting service=========");

		TransitPath transitPath = restTemplate.getForObject(REST_URI, TransitPath.class, originUnLocode,
