...
import com.javamonks.booking.domain.model.valueobjects.BookingAmount;
import com.javamonks.booking.domain.model.valueobjects.CargoItinerary;
import com.javamonks.booking.domain.model.valueobjects.Delivery;
import com.javamonks.booking.domain.model.valueobjects.LastCargoHandledEvent;
...
import com.javamonks.shareddomain.model.events.CargoBookedEventData;
import com.javamonks.shareddomain.model.events.CargoRoutedEvent;
import com.javamonks.shareddomain.model.events.CargoRoutedEventData;

@Entity
@NamedQueries({ @NamedQuery(name = "Cargo.findAll", query = "Select c from Cargo c"),
		@NamedQuery(name = "Cargo.findByBookingId", query = "Select c from Cargo c where c.bookingId.bookingId = :bookingId"),
		@NamedQuery(name = "Cargo.findAllBookingIds", query = "Select c.bookingId from Cargo c") })
public class Cargo extends AbstractAggregateRoot<Cargo> {
	...
	@Embedded
	private Delivery delivery; // Checks the delivery progress of the cargo against the actual Route
	...
	/**
	 * Constructor Command Handler for a new Cargo booking. Sets the state of the
	 * Aggregate and registers the Cargo Booked Event
	 *
	 */
	public Cargo(BookCargoCommand bookCargoCommand) {
		this.bookingId = new BookingId(bookCargoCommand.getBookingId());
		this.routeSpecification = new RouteSpecification(new Location(bookCargoCommand.getOriginLocation()),
				new Location(bookCargoCommand.getDestLocation()), bookCargoCommand.getDestArrivalDeadline());
	...
	/**
	 * Command Handler for the Route Cargo Command. Sets the state of the Aggregate
	 * and registers the Cargo routed event
	 * 
